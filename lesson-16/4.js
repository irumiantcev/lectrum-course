...
class Stringer {
    ...
    uppercaseFirst(string) {
        const firstLetterUpper = string[0].toUpperCase();
        const otherLetters = string.slice(1);
        const result = `${firstLetterUpper}${otherLetters}`;
    ...
    uppercaseAll(string) {
        ...
        const result = stringToArray.map((value) => this.uppercaseFirst(value)).join(' ');
